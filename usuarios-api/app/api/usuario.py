...
from app.services.usuario_service import criar_usuario_service, buscar_usuario_por_nome_service, buscar_usuario_por_id_service
...
@router.get("/id/{id}", response_model=UsuarioRead)
def buscar_usuario_por_id(id: int, session: Session = Depends(get_session)):
    usuario = buscar_usuario_por_id_service(session, id)
    if not usuario:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    return usuario
